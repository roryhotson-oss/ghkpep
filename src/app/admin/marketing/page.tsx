...
export default function MarketingPage() {
  ...
  return <div className="p-6 lg:p-8 max-w-6xl">
    ...
    <form onSubmit={createDraft} className="bg-[#141414] border border-[#2b3538] rounded-xl p-6 mb-8 flex flex-col md:flex-row gap-3 md:items-end">
      <label className="flex-1 text-sm text-[#a7b0b2]">Product<select required value={productSlug} onChange={(event) => setProductSlug(event.target.value)} className="mt-2 w-full bg-[#1a1a1a] border border-[#2b3538] rounded-lg px-3 py-3 text-white"><option value="">Select a product</option>{products.map((product) => <option key={product.slug} value={product.slug}>{product.name}</option>)}</select></label>
      <label className="md:w-44 text-sm text-[#a7b0b2]">Platform<select value={platform} onChange={(event) => setPlatform(event.target.value)} className="mt-2 w-full bg-[#1a1a1a] border border-[#2b3538] rounded-lg px-3 py-3 text-white"><option value="x">X</option><option value="facebook">Facebook</option><option value="reddit">Reddit</option><option value="instagram">Instagram</option><option value="linkedin">LinkedIn</option></select></label>
      <label className="md:w-56 text-sm text-[#a7b0b2]">Schedule time<input type="datetime-local" value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} className="mt-2 w-full bg-[#1a1a1a] border border-[#2b3538] rounded-lg px-3 py-3 text-white" /></label>
      <button type="submit" className="px-5 py-3 bg-[#21c7a5] text-black rounded-lg font-bold">Create Draft</button>
      {status && <span className="text-sm text-[#a7b0b2]">{status}</span>}
    </form>
    <div className="space-y-4">{posts.length === 0 ? <div className="bg-[#141414] border border-[#2b3538] rounded-xl p-8 text-[#a7b0b2]">No marketing drafts yet.</div> : posts.map((post) => <article key={post.id} className="bg-[#141414] border border-[#2b3538] rounded-xl p-5"><div className="flex flex-wrap items-center justify-between gap-3 mb-3"><div><span className="text-[#21c7a5] font-bold uppercase text-xs">{post.platform}</span><span className="text-[#7b898e] text-xs ml-3">{post.productSlug}</span></div><div className="flex gap-2"><select value={post.status} onChange={(event) => updateStatus(post.id, event.target.value)} className="bg-[#1a1a1a] border border-[#2b3538] rounded-lg px-3 py-2 text-xs text-white"><option value="draft">Draft</option><option value="approved">Approved</option><option value="scheduled">Scheduled</option><option value="published">Published</option><option value="failed">Failed</option></select>{['approved', 'scheduled'].includes(post.status) && <button type="button" onClick={() => publish(post.id)} className="px-3 py-2 bg-[#21c7a5] text-black rounded-lg text-xs font-bold">Publish now</button>}</div></div><p className="text-[#e1e7e5] text-sm leading-relaxed">{post.content}</p>{post.imageUrl && <p className="text-[#7b898e] text-xs mt-3">Image: {post.imageUrl}</p>}</article>)}</div>
  </div>;
}
