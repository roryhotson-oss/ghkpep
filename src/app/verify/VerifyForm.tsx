    ...
    <div>
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          className="flex-1 bg-[#141414] border border-[#2b3538] rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#8298aa]"
        />
        <button type="submit" disabled={loading} className="px-6 py-3 bg-[#8298aa] text-black font-semibold rounded-lg border border-[#aebfca] hover:bg-[#657c8f] transition text-sm disabled:opacity-60">
          {loading ? 'Checking...' : 'Verify'}
        </button>
      </form>

      {status === 'found' && result && (
        <div className="mt-6 rounded-lg border border-[#657c8f] bg-[#17232d] p-5">
          <p className="text-[#a6b8c4] text-sm font-semibold">Lot found</p>
          <h2 className="text-lg font-bold mt-1">{result.name}</h2>
          <p className="text-[#a7b0b2] text-sm mt-1">Lot {result.lot} is listed with batch documentation for research and chemistry use.</p>
          <a href={`/api/coa?lot=${encodeURIComponent(result.lot)}`} target="_blank" rel="noreferrer" className="inline-flex mt-4 rounded-md border border-[#aebfca] bg-[#9aafbd]/30 px-4 py-2 text-sm font-semibold text-[#d8e2e8] hover:bg-[#9aafbd]/50 transition">
            View Test Report
          </a>
        </div>
      )}
